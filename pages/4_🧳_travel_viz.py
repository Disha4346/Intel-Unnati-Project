...
def get_pics_from_location(locations_df, size=10):
    """Get images from Unsplash using the GPS coordinates"""
    ...
    for index, row in locations_df.iterrows():
        try:
            # Search for photos near the given coordinates
            response = requests.get(
                'https://api.unsplash.com/search/photos',
                headers=headers,
                params={
                    'query': 'landscape',  # You can customize this query
                    'per_page': size,
                    'page': 1,
            ...
            if data['total'] > 0:
                choice_max = min(size - 1, len(data['results']) - 1)
                selection = random.randint(0, choice_max)
                selected_photo = data['results'][selection]
